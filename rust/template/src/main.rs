...
fn main() -> Result<(), String> {
    let parser = opts! {
        ...
        auto_shorts false;
        opt store:bool=true, desc:"Do not store output relation state. 'dump' and 'dump <table>' commands will produce no output."; // --no-opt
        ...
        opt clock:bool=false, short:'c', desc:"management second clock.";                                                           // --clock
        opt recording:Option<String>, short:'r', desc:"record command trace to file.";                                         // --recording
    ...
    // xxx - pass config through - a map or somesuch? - we really just want argv :)
    #[cfg(feature = "distribution")]
    d3main::start_d3log(
        args.debug_management,
        args.recording,
        args.clock,
        args.input,
    )
